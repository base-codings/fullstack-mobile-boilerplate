...
> 🌐 **Language:** **English** · [Tiếng Việt](languages/vi/system-architecture.md) · [中文](languages/zh/system-architecture.md) · [한국어](languages/ko/system-architecture.md)
...
└────────┬────────┘
         │
         │ Dio HTTP Client (auto-generated from OpenAPI spec)
         │
┌────────▼────────────────────────────────────────────┐
...
All API responses (success or error) wrap data in a standard envelope:
...
| **Repository** | Data access abstraction (optional if using Prisma directly) | Interface + implementation |
| **Prisma** | Type-safe database queries, migrations | `prisma.user.findUnique({ where: { id } })` |
| **Guard/Interceptor** | Cross-cutting: auth, logging, rate limiting | Applied globally or per-route via decorators |
...
### Folder Structure (apps/mobile/lib)
...
Each feature has an async controller:
...
### Local Development
- **Backend:** `SKIP_DB=true` (hello endpoint only) or real Postgres
- **Mobile:** `.env` with `API_BASE_URL=auto` → localhost:3000 (auto-resolved per platform)

### Staging
- **Backend:** Supabase staging database, real auth (JwtAuthGuard)
- **Mobile:** `.env` with explicit staging API URL or `flutter run --dart-define=API_BASE_URL=...`
...
| Mechanism | Implementation | Enforced At |
|---|---|---|
