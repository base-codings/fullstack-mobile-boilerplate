...
> 🌐 **Ngôn ngữ:** [English](../../code-standards.md) · **Tiếng Việt** · [中文](../zh/code-standards.md) · [한국어](../ko/code-standards.md)

## Tổ Chức & Đặt Tên File

### Quy Ước Đặt Tên
- **Files:** `kebab-case` (ví dụ: `hello-controller.ts`, `hello_repository.dart`)
- **Directories:** `kebab-case` (ví dụ: `src/modules/user-management/`, `lib/features/auth/`)
- **Classes/Interfaces:** `PascalCase` (ví dụ: `HelloController`, `UserRepository`)
- **Functions/Variables:** `camelCase` (ví dụ: `getHello`, `userId`)
- **Constants:** `UPPER_SNAKE_CASE` (ví dụ: `MAX_RETRY_COUNT`, `API_TIMEOUT_MS`)
- **Private members:** Prefix with `_` (ví dụ: `_internal`, `_service`)

### Giới Hạn Kích Thước File

**Tối đa 200 dòng mỗi file** (không tính comments). Lý do: tăng khả năng đọc, khuyến khích single responsibility, dễ dàng cho LLM tools phân tích.

**Mẫu vi phạm:** Nếu file tiến tới 180 dòng, lên kế hoạch modularization trong sprint tiếp theo.
