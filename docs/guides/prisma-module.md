# Prisma Module Recipe

> 🌐 **Language:** **English** · [Tiếng Việt](../languages/vi/guides/prisma-module.md) · [中文](../languages/zh/guides/prisma-module.md) · [한국어](../languages/ko/guides/prisma-module.md)

This guide covers extending the database schema with Prisma and wiring services to query the new models.

## Prerequisites

- Boilerplate set up locally
- `DATABASE_URL` and `DIRECT_URL` configured in `.env` (or `SKIP_DB=true` for schema-only work)
...
export class PostsController {
  ...
  async findAll(): Promise<PostResponseDto[]> {
    return this.postsService.findAll(true);  // public posts only
  }
...
3. **Test container** (advanced):
   - Use testcontainers library to spin up Postgres per test suite
...
See [flutter-feature.md](./flutter-feature.md) to create a Flutter feature that calls these endpoints.
...
**Next Steps:**
- [Add New Backend Module](./backend-module.md) — complete recipe with auth/envelope
- [API Contract Workflow](./api-contract.md) — when DTOs change
