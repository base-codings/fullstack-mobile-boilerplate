...
> 🌐 **Language:** **English** · [Tiếng Việt](languages/vi/feature-boundaries.md) · [中文](languages/zh/feature-boundaries.md) · [한국어](languages/ko/feature-boundaries.md)

## Core Principle

**Loose coupling, high cohesion.** Features and modules are isolated; cross-feature concerns live in `core/` (backend) or `core/` / `shared/` (frontend).
...
Each NestJS module may have an `internal/` directory. **Other modules cannot import from it.**
...
Each feature is independent. Feature A cannot import from Feature B.
